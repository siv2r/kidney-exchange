

<div class='input-field'> 
  <div class="label-box">
    <label>HLA Antigens</label>
  </div>
  <div class="input-box">
    <div class="antigen">

      <label for="r_hla_a"> A
        <label class="required">*</label>
      </label>
      
      <select class="beautify requiredField single" id="r_hla_a" name="r_hla_a" multiple>  <!-- added requiredField class -->
        <option value="None">None</option>
        <option value="A1">A1</option>
        <option value="A2">A2</option>
        <option value="A203">A203</option>
        <option value="A210">A210</option>
        <option value="A3">A3	</option>
        <option value="A9">A9	</option>
        <option value="A10">A10</option>		 
        <option value="A11">A11</option>		 
        <option value="A19">A19</option>			 
        <option value="A23(9)">A23(9)</option>	 	 
        <option value="A24(9)">A24(9)</option>	 	 
        <option value="A2403">A2403</option>	 	 
        <option value="A25(10)">A25(10)</option>	 	 
        <option value="A26(10)">A26(10)</option>	 	 
        <option value="A28">A28</option>	 
        <option value="A29(19)">A29(19)</option>	 	 
        <option value="A30(19)">A30(19)</option>	 	 
        <option value="A31(19)">A31(19)</option>	 	 
        <option value="A32(19)">A32(19)</option>	 
        <option value="A33(19)">A33(19)</option> 	 
        <option value="A34(10)">A34(10)</option> 
        <option value="A36">A36</option>
        <option value="A43">A43</option>	 	 
        <option value="A66(10)">A66(10)</option>	 
        <option value="A68(28)">A68(28)</option>	 
        <option value="A69(28)">A69(28)</option> 	 
        <option value="A74(19)">A74(19)</option>
        <option value="A80">A80</option>
      </select>
    </div>


    <div class="antigen">
      <label for="r_hla_b"> B
        <label class="required">*</label>
      </label>

      <select class="beautify requiredField single" id="r_hla_b" name="r_hla_b" multiple> <!-- added requiredField class -->
      <option value="None">None</option>	
      <option value="B5">B5</option>	
      <option value="B7">B7	</option>
      <option value="B703">B703</option>
      <option value="B8">B8	</option>
      <option value="B12">B12</option>	
      <option value="B13">B13</option>	
      <option value="B14">B14</option>		 
      <option value="B15">B15</option>		 
      <option value="B16">B16</option>		 
      <option value="B17">B17</option>	
      <option value="B18">B18</option>		 	 
      <option value="B21">B21</option>		 	 
      <option value="B22">B22</option>		 	 
      <option value="B27">B27</option>		 	 
      <option value="B2708">B2708</option>	 	 
      <option value="B35">B35</option>	 	 	 
      <option value="B37">B37</option>	 	 	 
      <option value="B38(16)">B38(16)</option>	 	 
      <option value="B39(16)">B39(16)</option> 	 
      <option value="B3901">B3901</option>	 	 	 
      <option value="B3901">B3901</option>	 	 	 
      <option value="B3901">B3901</option>	 	 	 
      <option value="B3902">B3902</option>	 	 	 
      <option value="B40">B40</option>	 		 	 	 
      <option value="B4005">B4005</option>		 	 
      <option value="B41">B41</option>	 	 	 
      <option value="B42">B42</option>	 	 	 
      <option value="B44(12)">B44(12)</option>	 	 	 
      <option value="B45(12)">B45(12)</option>	 	 	 	 	 
      <option value="B46">B46</option>	 	 	 	 	 
      <option value="B47">B47</option>	 	 	 	 	 
      <option value="B48">B48</option>	 	 	 	 	 
      <option value="B49(21)">B49(21)</option>	 	 	 	 	 
      <option value="B50(21)">B50(21)</option>	 	 	 	 	 
      <option value="B51(5)">B51(5)</option>	 	 	 	 	 
      <option value="B5102">B5102</option>	 	 	 	 	 
      <option value="B5103">B5103</option>	 	 	 	 	 
      <option value="B52(5)">B52(5)</option>	 	 	 	 	 
      <option value="B53">B53</option>	 	 	 	 	 
      <option value="B54(22)">B54(22)</option>	 	 	 	 	 
      <option value="B55(22)">B55(22)</option>	 	 	 	 	 
      <option value="B56(22)">B56(22)</option>	 	 	 	 	 
      <option value="B57(17)">B57(17)</option>	 	 	 	 	 
      <option value="B58(17)">B58(17)</option>	 	 	 	 	 
      <option value="B59">B59</option>	 	 	 	 	 
      <option value="B60(40)">B60(40)</option>	 	 	 	 	 
      <option value="B61(40)">B61(40)</option>	 	 	 	 	 
      <option value="B62(15)">B62(15)</option>	 	 	 	 	 
      <option value="B63(15)">B63(15)</option>	 	 	 	 	 
      <option value="B64(14)">B64(14)</option>	 	 	 	 	 
      <option value="B65(14)">B65(14)</option>	 	 	 	 	 
      <option value="B67">B67</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_hla_dr">DR
        <label class="required">*</label>
      </label>

      <select class="beautify single requiredField" id="r_hla_dr" name="r_hla_dr" multiple>   <!-- added requiredField class -->
      <option value="None">None</option>
        <option value="DR1">DR1</option>
        <option value="DR103">DR103</option>
        <option value="DR2">DR2</option>
        <option value="DR3">DR3</option>
        <option value="DR4">DR4</option>
        <option value="DR5">DR5</option>
        <option value="DR6">DR6</option>
        <option value="DR7">DR7</option>
        <option value="DR8">DR8</option>
        <option value="DR9">DR9</option>
        <option value="DR10">DR10</option>
        <option value="DR11(5)">DR11(5)</option>	 	 
        <option value="DR12(5)">DR12(5)</option>	 	 
        <option value="DR13(6)">DR13(6)</option>	 	 
        <option value="DR14(6)">DR14(6)</option>	 	 
        <option value="DR1403">DR1403</option>	 	 
        <option value="DR1404">DR1404</option>	 	 
        <option value="DR15(2)">DR15(2)</option>	 	 
        <option value="DR16(2)">DR16(2)</option>	 	 
        <option value="DR17(3)">DR17(3)</option>	 	 
        <option value="DR18(3)">DR18(3)</option>	 	 	 
        <option value="DR51">DR51</option>	 	 
        <option value="DR52">DR52</option>	 	 
        <option value="DR53">DR53</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_hla_c"> C</label>
      <select class="beautify single" id="r_hla_c" name="r_hla_c[]" multiple>
        <option value="None">None</option>
        <option value="Cw1">Cw1</option>	
        <option value="Cw2">Cw2</option>	
        <option value="Cw3">Cw3</option>      
        <option value="Cw4">Cw4</option>      
        <option value="Cw5">Cw5</option>      
        <option value="Cw6">Cw6</option>      
        <option value="Cw7">Cw7</option>      
        <option value="Cw8">Cw8</option>      
        <option value="Cw9(w3)">Cw9(w3)</option>      
        <option value="Cw10(w3)">Cw10(w3)</option>
      </select>
    </div>


    <div class="antigen">
      <label for="r_hla_d" > D</label>
      <select class="beautify single" id="r_hla_d" name="r_hla_d[]" multiple>
      <option value="None">None</option>
        <option value="Dw1">Dw1</option>
        <option value="Dw2">Dw2</option>
        <option value="Dw3">Dw3</option>
        <option value="Dw4">Dw4</option>
        <option value="Dw5">Dw5</option>
        <option value="Dw6">Dw6</option>
        <option value="Dw7">Dw7</option>
        <option value="Dw8">Dw8</option>
        <option value="Dw9">Dw9</option>
        <option value="Dw10">Dw10</option>
        <option value="Dw11(w7)">Dw11(w7)</option>		 	 
        <option value="Dw12">Dw12</option>		 	 
        <option value="Dw13">Dw13</option>		 	 
        <option value="Dw14">Dw14</option>		 	 
        <option value="Dw15">Dw15</option>		 	 
        <option value="Dw16">Dw16</option>		 	 
        <option value="Dw17(w7)">Dw17(w7)</option>		 	 
        <option value="Dw18(w6)">Dw18(w6)</option>		 	 
        <option value="Dw19(w6)">Dw19(w6)</option>	 	 
        <option value="Dw20">Dw20</option>		 	 
        <option value="Dw21">Dw21</option>		 	 
        <option value="Dw22">Dw22</option>	 	 	 
        <option value="Dw23">Dw23</option>		 	 
        <option value="Dw24">Dw24</option>		 	 
        <option value="Dw25">Dw25</option>		 	 
        <option value="Dw26">Dw26</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_hla_dq">DQ</label>
      <select class="beautify single" id="r_hla_dq" name="r_hla_dq[]" multiple>
      <option value="None">None</option>
        <option value="DQ1">DQ1</option>	
        <option value="DQ2">DQ2</option>	
        <option value="DQ3">DQ3</option>	
        <option value="DQ4">DQ4</option>	
        <option value="DQ5(1)">DQ5(1)</option>	
        <option value="DQ6(1)">DQ6(1)</option>	
        <option value="DQ7(3)">DQ7(3)</option>	 
        <option value="DQ8(3)">DQ8(3)</option>	 
        <option value="DQ9(3)">DQ9(3)</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_hla_dp">DP</label>
      <select class="beautify single" id="r_hla_dp" name="r_hla_dp[]" multiple>
      <option value="None">None</option>
        <option value="DPw1">DPw1</option>
        <option value="DPw2">DPw2</option>
        <option value="DPw3">DPw3</option>
        <option value="DPw4">DPw4</option>
        <option value="DPw5">DPw5</option>
        <option value="DPw6">DPw6</option>
      </select>
    </div>
  </div>
</div>


<div class="input-field">
  <div class="label-box">
    <label for="r_hla-report" class="label-box">HLA report</label>
    <label class="required">* </label>
  </div>     
  <div class="input-box">
    <input type="file" id="r_hla-report" name="r_hla-report" class="requiredField single">
  </div>
</div>


<div class='input-field'> 
  <div class="label-box">
    <label>Unacceptable Antigens</label>
  </div>

  <div class="input-box">
    <div class="antigen">
      <label for="r_ua_a"> A</label>
      <select class="beautify single" id="r_ua_a" name="r_ua_a[]" multiple>
        <option value="None">None</option>
        <option value="A1">A1</option>
        <option value="A2">A2</option>
        <option value="A203">A203</option>
        <option value="A210">A210</option>
        <option value="A3">A3	</option>
        <option value="A9">A9	</option>
        <option value="A10">A10</option>		 
        <option value="A11">A11</option>		 
        <option value="A19">A19</option>			 
        <option value="A23(9)">A23(9)</option>	 	 
        <option value="A24(9)">A24(9)</option>	 	 
        <option value="A2403">A2403</option>	 	 
        <option value="A25(10)">A25(10)</option>	 	 
        <option value="A26(10)">A26(10)</option>	 	 
        <option value="A28">A28</option>	 
        <option value="A29(19)">A29(19)</option>	 	 
        <option value="A30(19)">A30(19)</option>	 	 
        <option value="A31(19)">A31(19)</option>	 	 
        <option value="A32(19)">A32(19)</option>	 
        <option value="A33(19)">A33(19)</option> 	 
        <option value="A34(10)">A34(10)</option> 
        <option value="A36">A36</option>
        <option value="A43">A43</option>	 	 
        <option value="A66(10)">A66(10)</option>	 
        <option value="A68(28)">A68(28)</option>	 
        <option value="A69(28)">A69(28)</option> 	 
        <option value="A74(19)">A74(19)</option>
        <option value="A80">A80</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_ua_b"> B</label>
      <select class="beautify single" id="r_ua_b" name="r_ua_b[]" multiple>
      <option value="None">None</option>	
      <option value="B5">B5</option>	
      <option value="B7">B7	</option>
      <option value="B703">B703</option>
      <option value="B8">B8	</option>
      <option value="B12">B12</option>	
      <option value="B13">B13</option>	
      <option value="B14">B14</option>		 
      <option value="B15">B15</option>		 
      <option value="B16">B16</option>		 
      <option value="B17">B17</option>	
      <option value="B18">B18</option>		 	 
      <option value="B21">B21</option>		 	 
      <option value="B22">B22</option>		 	 
      <option value="B27">B27</option>		 	 
      <option value="B2708">B2708</option>	 	 
      <option value="B35">B35</option>	 	 	 
      <option value="B37">B37</option>	 	 	 
      <option value="B38(16)">B38(16)</option>	 	 
      <option value="B39(16)">B39(16)</option> 	 
      <option value="B3901">B3901</option>	 	 	 
      <option value="B3901">B3901</option>	 	 	 
      <option value="B3901">B3901</option>	 	 	 
      <option value="B3902">B3902</option>	 	 	 
      <option value="B40">B40</option>	 		 	 	 
      <option value="B4005">B4005</option>		 	 
      <option value="B41">B41</option>	 	 	 
      <option value="B42">B42</option>	 	 	 
      <option value="B44(12)">B44(12)</option>	 	 	 
      <option value="B45(12)">B45(12)</option>	 	 	 	 	 
      <option value="B46">B46</option>	 	 	 	 	 
      <option value="B47">B47</option>	 	 	 	 	 
      <option value="B48">B48</option>	 	 	 	 	 
      <option value="B49(21)">B49(21)</option>	 	 	 	 	 
      <option value="B50(21)">B50(21)</option>	 	 	 	 	 
      <option value="B51(5)">B51(5)</option>	 	 	 	 	 
      <option value="B5102">B5102</option>	 	 	 	 	 
      <option value="B5103">B5103</option>	 	 	 	 	 
      <option value="B52(5)">B52(5)</option>	 	 	 	 	 
      <option value="B53">B53</option>	 	 	 	 	 
      <option value="B54(22)">B54(22)</option>	 	 	 	 	 
      <option value="B55(22)">B55(22)</option>	 	 	 	 	 
      <option value="B56(22)">B56(22)</option>	 	 	 	 	 
      <option value="B57(17)">B57(17)</option>	 	 	 	 	 
      <option value="B58(17)">B58(17)</option>	 	 	 	 	 
      <option value="B59">B59</option>	 	 	 	 	 
      <option value="B60(40)">B60(40)</option>	 	 	 	 	 
      <option value="B61(40)">B61(40)</option>	 	 	 	 	 
      <option value="B62(15)">B62(15)</option>	 	 	 	 	 
      <option value="B63(15)">B63(15)</option>	 	 	 	 	 
      <option value="B64(14)">B64(14)</option>	 	 	 	 	 
      <option value="B65(14)">B65(14)</option>	 	 	 	 	 
      <option value="B67">B67</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_ua_dr">DR</label>
      <select class="beautify single" id="r_ua_dr" name="r_ua_dr[]" multiple>
      <option value="None">None</option>
        <option value="DR1">DR1</option>
        <option value="DR103">DR103</option>
        <option value="DR2">DR2</option>
        <option value="DR3">DR3</option>
        <option value="DR4">DR4</option>
        <option value="DR5">DR5</option>
        <option value="DR6">DR6</option>
        <option value="DR7">DR7</option>
        <option value="DR8">DR8</option>
        <option value="DR9">DR9</option>
        <option value="DR10">DR10</option>
        <option value="DR11(5)">DR11(5)</option>	 	 
        <option value="DR12(5)">DR12(5)</option>	 	 
        <option value="DR13(6)">DR13(6)</option>	 	 
        <option value="DR14(6)">DR14(6)</option>	 	 
        <option value="DR1403">DR1403</option>	 	 
        <option value="DR1404">DR1404</option>	 	 
        <option value="DR15(2)">DR15(2)</option>	 	 
        <option value="DR16(2)">DR16(2)</option>	 	 
        <option value="DR17(3)">DR17(3)</option>	 	 
        <option value="DR18(3)">DR18(3)</option>	 	 	 
        <option value="DR51">DR51</option>	 	 
        <option value="DR52">DR52</option>	 	 
        <option value="DR53">DR53</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_ua_c"> C</label>
      <select class="beautify single" id="r_ua_c" name="r_ua_c[]" multiple>
      <option value="None">None</option>
        <option value="Cw1">Cw1</option>	
        <option value="Cw2">Cw2</option>	
        <option value="Cw3">Cw3</option>      
        <option value="Cw4">Cw4</option>      
        <option value="Cw5">Cw5</option>      
        <option value="Cw6">Cw6</option>      
        <option value="Cw7">Cw7</option>      
        <option value="Cw8">Cw8</option>      
        <option value="Cw9(w3)">Cw9(w3)</option>      
        <option value="Cw10(w3)">Cw10(w3)</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_ua_d" > D</label>
      <select class="beautify single" id="r_ua_d" name="r_ua_d[]" multiple>
      <option value="None">None</option>
        <option value="Dw1">Dw1</option>
        <option value="Dw2">Dw2</option>
        <option value="Dw3">Dw3</option>
        <option value="Dw4">Dw4</option>
        <option value="Dw5">Dw5</option>
        <option value="Dw6">Dw6</option>
        <option value="Dw7">Dw7</option>
        <option value="Dw8">Dw8</option>
        <option value="Dw9">Dw9</option>
        <option value="Dw10">Dw10</option>
        <option value="Dw11(w7)">Dw11(w7)</option>		 	 
        <option value="Dw12">Dw12</option>		 	 
        <option value="Dw13">Dw13</option>		 	 
        <option value="Dw14">Dw14</option>		 	 
        <option value="Dw15">Dw15</option>		 	 
        <option value="Dw16">Dw16</option>		 	 
        <option value="Dw17(w7)">Dw17(w7)</option>		 	 
        <option value="Dw18(w6)">Dw18(w6)</option>		 	 
        <option value="Dw19(w6)">Dw19(w6)</option>	 	 
        <option value="Dw20">Dw20</option>		 	 
        <option value="Dw21">Dw21</option>		 	 
        <option value="Dw22">Dw22</option>	 	 	 
        <option value="Dw23">Dw23</option>		 	 
        <option value="Dw24">Dw24</option>		 	 
        <option value="Dw25">Dw25</option>		 	 
        <option value="Dw26">Dw26</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_ua_dq">DQ</label>
      <select class="beautify single" id="r_ua_dq" name="r_ua_dq[]" multiple>
      <option value="None">None</option>
        <option value="DQ1">DQ1</option>	
        <option value="DQ2">DQ2</option>	
        <option value="DQ3">DQ3</option>	
        <option value="DQ4">DQ4</option>	
        <option value="DQ5(1)">DQ5(1)</option>	
        <option value="DQ6(1)">DQ6(1)</option>	
        <option value="DQ7(3)">DQ7(3)</option>	 
        <option value="DQ8(3)">DQ8(3)</option>	 
        <option value="DQ9(3)">DQ9(3)</option>
      </select>
    </div>

    <div class="antigen">
      <label for="r_ua_dp">DP</label>
      <select class="beautify single" id="r_ua_dp" name="r_ua_dp[]" multiple>
      <option value="None">None</option>
        <option value="DPw1">DPw1</option>
        <option value="DPw2">DPw2</option>
        <option value="DPw3">DPw3</option>
        <option value="DPw4">DPw4</option>
        <option value="DPw5">DPw5</option>
        <option value="DPw6">DPw6</option>
      </select>
    </div>
  </div>
</div>

<div class="input-field">
  <div class="label-box">
    <label for="r_ua-report">DSA report</label>  
    <label class="required">* </label>
  </div>   
  <div class="input-box">
    <input type="file" id="r_ua-report" name="r_ua-report" class="requiredField single">
  </div>
</div>


<!-- 
DR11(5)	 	 
DR12(5)	 	 
DR13(6)	 	 
DR14(6)	 	 
DR1403	 	 
DR1404	 	 
DR15(2)	 	 
DR16(2)	 	 
DR17(3)	 	 
DR18(3)	 	 	 
DR51	 	 
DR52	 	 
DR53-->